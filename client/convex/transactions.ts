...
async function handleCheckoutSessionCompleted(ctx: ActionCtx, checkoutSession: Stripe.Checkout.Session) {
...
async function handleSubscriptionUpdate(ctx: ActionCtx, subscription: Stripe.Subscription) {
    ...
    const product = await stripe.products.retrieve(subscription.items.data[0].plan.product as string) as Stripe.Product;
    const interval = subscription.items.data[0].plan.interval;
    const user = await ctx.runQuery(internal.userProfiles.getByEmailInternal, { email: customer.email! });
    if (!isDefined(user)) {
        throw new ConvexError({
            code: "UserNotFound",
            message: "User not found",
    ...
    // payment successful
    if (status === "active") {
        const planName = product.name === "Basic Plan" ? PLANS.basic.name : product.name === "Premium Plan" ? PLANS.premium.name : product.name === "Enterprise Plan" ? PLANS.enterprise.name : PLANS.free.name;
        const refreshDate = interval === "month" ? current_period_end : get30DaysFromNowInSeconds(current_period_start);
        let minutesRemaining = user.minutesRemaining || 0;
        if (user.subscriptionStatus !== "active") {
            // new subscription or returning from cancelled
            minutesRemaining += planName === "basic" ? PLANS.basic.minutes : planName === "premium" ? PLANS.premium.minutes : planName === "enterprise" ? PLANS.enterprise.minutes : PLANS.free.minutes;
