...
async def assistant(
    state: BackgroundStageState, config: RunnableConfig, writer: StreamWriter
):
    ...
    if tool_call_detected:
        return get_stage_confirmation_tool_call_state_patch(
            StageTypes.BACKGROUND, function_name, state
        )
...
def create_graph():
    return (
        StateGraph(BackgroundStageState, AgentConfig)
        # nodes
