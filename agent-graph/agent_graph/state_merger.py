...
class StateMergerEventEmitter(EventEmitter[EventTypes], Generic[TState]):
    """An event emitter for state merger events"""


class StateMerger(StateMergerEventEmitter[TState]):
    ...
    async def connect(self, storage: StateStorage):
        ...
        with pf.interval("state_merger.connect"):
    ...
    async def _merge_initial_state(self, state: TState | Dict[str, Any]):
        ...
        with pf.interval("state_merger.merge_initial_state"):
