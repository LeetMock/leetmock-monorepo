...
class OptimalSolutionEvaluation(BaseModel):
    score: int = Field(
        description="Score candidate will receive for how well they provide an optimal solution"
    )
    comment: str = Field(description="Detailed feedback on optimal solution clarity")
    examples: List[str] = Field(
        description="Examples from code supporting the evaluation"
    )


class OptimizationProcessEvaluation(BaseModel):
    score: int = Field(
        description="Score candidate will receive for their optimization process"
    )
    comment: str = Field(description="Detailed feedback on optimization process")
    examples: List[str] = Field(
        description="Examples from conversation supporting the evaluation"
    )
