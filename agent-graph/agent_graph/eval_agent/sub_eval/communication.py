...
class ClarificationEvaluation(BaseModel):
    score: int = Field(
        description="Score candidate will receive for how well they asked relevant questions and seek clarification about requirements"
    )
    comment: str = Field(description="Detailed feedback on clarification skills")
    examples: List[str] = Field(
        description="Specific examples from the conversation to support the score and comment"
    )


class ThoughtProcessEvaluation(BaseModel):
    score: int = Field(
        description="Score candidate will receive for how well they explained their thought process"
    )
