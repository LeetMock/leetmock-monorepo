...
from agent_graph.eval_agent.prompts import (
    syntaxError_prompt, codeQuality_prompt, 
    codingSpeed_prompt
)
...
def evaluate_coding_speed(state: AgentState, config: RunnableConfig) -> dict:
    ...
    prompt = PromptTemplate(
        ...
        input_variables=["coding_problem", "candidate_code", "max_points", "interview_duration"]
    )
    ...
    interview_duration = time_diff_in_minutes(state.SESSION['sessionStartTime'], state.SESSION['sessionEndTime'])
    result = chain.invoke({
        "coding_problem": state.QUESTION['question'] if state.QUESTION else "",
        "candidate_code": state.SESSION_STATE['editor']['content'] if state.SESSION_STATE else "",
        "max_points": MAX_POINTS,
...
def technical_competency_final_evaluation(state: AgentState, config: RunnableConfig) -> dict:
